...
from offlinectl.bundle.bundle import read_meta
...
def extract_archive(archive_path: Path) -> Path:
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="offlinectl-bundle-"))
...
def detect_bundle(path: Path) -> Iterator[Path]:
    ...
    try:
        ...
        if is_temp:
            ...
            if target_dir.parent.name.startswith("offlinectl-bundle-"):
