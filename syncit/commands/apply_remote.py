"""syncit apply-remote — zero-dependency remote apply via SSH."""
        ...
        for host_id, host in hosts:
            ...
            with subprocess.Popen(
                ...
                if proc.returncode != 0:
                    ...
                    # the old code that tried to run 'syncit'.
                    if proc.returncode == 127:
                        err_console.print(
                            "[yellow]Hint: If you see 'syncit: command not found', ensure you are using 'uv run syncit' on the jumphost to run the latest code.[/yellow]"
                        )
                    errors.append(host_id)
                    host_failed = True
                else:
